...
// --- CORE GIOCHI (FUNZIONAMENTO IMMEDIATO) ---
window.openGame = function(gameName) {
    ...
    if (['cosciotto', 'simon', 'campanelle', 'boccale', 'bevuta', 'equilibrio'].includes(gameName)) {
        stage.classList.add('theme-wood');
    } else if (gameName === 'freccette') {
        stage.classList.add('theme-wall');
    } else if (gameName === 'barili') {
...
window.startGameLogic = function() {
    ...
    if (currentGame !== 'ratti' && currentGame !== 'simon' && currentGame !== 'campanelle') {
...
function initCosciotto() {
    ...
    stage.onpointermove = (e) => move(e.clientX);
        ...
        function fall() {
            ...
            if (top > stageH - 80 && top < stageH - 10) {
                ...
                if (iR.right > bR.left + 10 && iR.left < bR.right - 10) {
                    if (isBomb) { loseLife(); } else { score += 10; updateHUD(); }
                    item.remove(); return;
                }
            }
            if (top > stageH) { if (!isBomb) loseLife(); item.remove(); } 
            else { item.style.top = (top + speed) + 'px'; gameAnimationFrames.push(requestAnimationFrame(fall)); }
        }
        gameAnimationFrames.push(requestAnimationFrame(fall));
    }, 800);
...
function initRatti() {
    ...
    for (let i = 0; i < 9; i++) html += `<div class="hole" onpointerdown="missRat(event)"><div class="mole" id="mole-${i}" onpointerdown="whack(event, this)"></div></div>`;
    html += '</div>';
    ...
    function peep() {
        ...
        if (mole.classList.contains('up')) { gameTimeouts.push(setTimeout(peep, 100)); return; }
        const isCat = Math.random() < 0.3; 
        mole.className = 'mole'; 
        if (isCat) { mole.innerText = "🐱"; mole.dataset.type = "cat"; } else { mole.innerText = "🐭"; mole.dataset.type = "rat"; }
        mole.classList.add('up'); 
        let stayTime = Math.max(450, 1000 - (score * 5));
        gameTimeouts.push(setTimeout(() => {
            if (!gameActive) return;
            mole.classList.remove('up');
            gameTimeouts.push(setTimeout(peep, Math.random() * 400 + 200));
        }, stayTime));
    }
    gameTimeouts.push(setTimeout(peep, 500));
}
window.whack = function (e, mole) {
    if (e) { e.preventDefault(); e.stopPropagation(); }
    if (!mole.classList.contains('up') || !gameActive) return;
    if (mole.dataset.type === "cat") { loseLife(); mole.innerText = "😾"; } 
    else { score += 10; mole.innerText = "💥"; updateHUD(); }
    mole.classList.remove('up');
};
window.missRat = function (e) { if (e) { e.preventDefault(); e.stopPropagation(); } if (gameActive) loseLife(); }
...
function initFreccette() {
    ...
    function animateDart() {
        ...
        let r = 100, x = Math.sin(angle) * r, y = Math.cos(angle * 1.5) * r;
        target.style.transform = `translate(calc(-50% + ${x}px), calc(-50% + ${y}px))`;
        target.dataset.x = x; target.dataset.y = y;
        gameAnimationFrames.push(requestAnimationFrame(animateDart));
    }
    gameAnimationFrames.push(requestAnimationFrame(animateDart));
}
window.throwDart = function (e) {
    ...
    const d = Math.sqrt(parseFloat(t.dataset.x||0)**2 + parseFloat(t.dataset.y||0)**2); 
...
function initBarili() {
    ...
    const world = document.getElementById('tower-world'), mover = document.getElementById('moving-block');
    let level = 0, w = 150, pos = 0, dir = 1, speed = 3, h = 25, stageW = stage.offsetWidth;
    mover.style.width = w + 'px'; mover.style.bottom = '0px';
    function animateBarrel() {
        if (!gameActive) return;
        pos += speed * dir;
        if (pos > stageW - w || pos < 0) dir *= -1;
        mover.style.left = pos + 'px';
        gameAnimationFrames.push(requestAnimationFrame(animateBarrel));
    }
    gameAnimationFrames.push(requestAnimationFrame(animateBarrel));
    stage.onpointerdown = function (e) {
        if (e.target.tagName === 'BUTTON' || !gameActive) return;
        let prevLeft = (stageW - 150) / 2, prevWidth = 150;
        if (level > 0) {
        ...
        if (level > 0) {
            const oL = Math.max(pos, prevLeft), oR = Math.min(pos + w, prevLeft + prevWidth);
...
function initSimon() {
    ...
    sSeq = []; sStep = 0; sClick = false; lastSimonClick = 0; gameTimeouts.push(setTimeout(playS, 1000));
}
function playS() {
    if (!gameActive) return;
    ...
    let i = 0, int = setInterval(() => {
        if (!gameActive) { clearInterval(int); return; }
...
function flashS(idx) {
    ...
    if (b[idx]) { b[idx].classList.add('active-light'); gameTimeouts.push(setTimeout(() => b[idx].classList.remove('active-light'), 300)); }
}
window.clkS = function (e, idx) {
    if (e) { e.preventDefault(); e.stopPropagation(); }
    let now = Date.now(); if (now - lastSimonClick < 300) return; lastSimonClick = now;
    if (!sClick || !gameActive) return;
    flashS(idx);
    if (idx !== sSeq[sStep]) { lives = 0; updateHUD(); gameOver(); return; } 
    sStep++;
    if (sStep >= sSeq.length) { score += sSeq.length * 10; updateHUD(); sClick = false; gameTimeouts.push(setTimeout(playS, 1000)); }
};

// --- GIOCO: BOCCALE ---
function initBoccale() {
    const stage = document.getElementById('game-stage');
    stage.innerHTML = `<div class="table-boccale"></div><div class="target-zone"></div><div id="sliding-mug" class="sliding-mug" style="bottom:10%;">🍺</div>`;
    const mug = document.getElementById('sliding-mug');
    let startY = 0, isDragging = false, velocity = 0, posY = 10;
    const handleStart = (y) => { if(gameActive && velocity === 0) { isDragging = true; startY = y; } };
    const handleEnd = (y) => { if(isDragging && gameActive) { isDragging = false; let dy = startY - y; if(dy > 0) { velocity = Math.min(dy * 0.04, 7); gameAnimationFrames.push(requestAnimationFrame(slideMug)); } } };
    stage.onpointerdown = (e) => handleStart(e.clientY); stage.onpointerup = (e) => handleEnd(e.clientY); stage.onpointercancel = (e) => handleEnd(e.clientY); 
    function slideMug() {
        if(!gameActive) return;
        posY += velocity; velocity *= 0.93; mug.style.bottom = posY + '%';
        if(velocity < 0.1) {
            velocity = 0;
            if(posY >= 70 && posY <= 90) { score += 50; flashStage('#34d399'); updateHUD(); } else { loseLife(); }
            gameTimeouts.push(setTimeout(() => { if(gameActive) { posY = 10; mug.style.bottom = '10%'; } }, 1000));
            return;
        }
        if(posY > 110) { velocity = 0; loseLife(); gameTimeouts.push(setTimeout(() => { if(gameActive) { posY = 10; mug.style.bottom = '10%'; } }, 1000)); return; }
        gameAnimationFrames.push(requestAnimationFrame(slideMug));
    }
}

// --- GIOCO: CAMPANELLE ---
function initCampanelle() {
    const stage = document.getElementById('game-stage');
    stage.innerHTML = `<div id="campanelle-msg" style="position:absolute; top:20px; width:100%; text-align:center; color:var(--gold); font-weight:bold; z-index:10; font-size:18px;">Guarda bene...</div>
        <div style="position:relative; width:100%; height:100%;">
            <div class="cup-wrap" id="cup-0" onclick="checkCup(0)"><div class="cup-icon">🍺</div><div class="coin-icon">🪙</div></div>
            <div class="cup-wrap" id="cup-1" onclick="checkCup(1)"><div class="cup-icon">🍺</div><div class="coin-icon">🪙</div></div>
            <div class="cup-wrap" id="cup-2" onclick="checkCup(2)"><div class="cup-icon">🍺</div><div class="coin-icon">🪙</div></div>
        </div>`;
    let coinPos = Math.floor(Math.random() * 3), canPick = false;
    const msg = document.getElementById('campanelle-msg'), posizioniX = [-90, 0, 90]; let statoCoppe = [0, 1, 2]; 
    for(let i=0; i<3; i++) document.getElementById(`cup-${i}`).style.transform = `translateX(calc(-50% + ${posizioniX[statoCoppe[i]]}px))`;
    const coppaMoneta = document.getElementById(`cup-${coinPos}`);
    coppaMoneta.querySelector('.cup-icon').style.transform = 'translateY(-50px)'; coppaMoneta.querySelector('.coin-icon').style.opacity = '1';

    gameTimeouts.push(setTimeout(() => {
        if(!gameActive) return;
        coppaMoneta.querySelector('.cup-icon').style.transform = 'translateY(0)'; coppaMoneta.querySelector('.coin-icon').style.opacity = '0';
        msg.innerText = "Mescolamento...";
        let scambi = 0, shuffleInterval = setInterval(() => {
            if(!gameActive) { clearInterval(shuffleInterval); return; }
            let a = Math.floor(Math.random() * 3), b = Math.floor(Math.random() * 3);
            while(a === b) { b = Math.floor(Math.random() * 3); }
            let temp = statoCoppe[a]; statoCoppe[a] = statoCoppe[b]; statoCoppe[b] = temp;
            document.getElementById(`cup-${a}`).style.transform = `translateX(calc(-50% + ${posizioniX[statoCoppe[a]]}px))`;
            document.getElementById(`cup-${b}`).style.transform = `translateX(calc(-50% + ${posizioniX[statoCoppe[b]]}px))`;
            scambi++;
            if(scambi > 8 + Math.floor(score/10)) { clearInterval(shuffleInterval); msg.innerText = "Dov'è la moneta?"; canPick = true; }
        }, 350 - Math.min(200, score * 2)); 
        gameIntervals.push(shuffleInterval);
    }, 1500));

    window.checkCup = function(idx) {
        if(!gameActive || !canPick) return; canPick = false;
        const targetCup = document.getElementById(`cup-${idx}`);
        targetCup.querySelector('.cup-icon').style.transform = 'translateY(-50px)';
        if(idx === coinPos) { targetCup.querySelector('.coin-icon').style.opacity = '1'; score += 20; updateHUD(); flashStage('#34d399'); } 
        else { loseLife(); document.getElementById(`cup-${coinPos}`).querySelector('.cup-icon').style.transform = 'translateY(-50px)'; document.getElementById(`cup-${coinPos}`).querySelector('.coin-icon').style.opacity = '1'; }
        gameTimeouts.push(setTimeout(() => { if(gameActive) initCampanelle(); }, 1500)); 
    }
}
...
function initBevuta() {
    const stage = document.getElementById('game-stage');
    stage.innerHTML = `<div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%;">
            <div class="glass-bevuta"><div id="liquid-level" class="liquid-level"></div></div>
            <div style="display:flex; gap:20px;"><button class="btn-action" onclick="drinkTap('L')" style="width:80px; padding:15px; font-size:24px;">👈</button><button class="btn-action" onclick="drinkTap('R')" style="width:80px; padding:15px; font-size:24px;">👉</button></div>
        </div>`;
    let lastTap = '', liquid = 100;
    window.drinkTap = function(side) {
        if(!gameActive) return;
        if(side === lastTap) { flashStage('#fbbf24'); liquid = Math.min(100, liquid + 5); document.getElementById('liquid-level').style.height = liquid + '%'; } 
        else {
            lastTap = side; liquid -= 10; document.getElementById('liquid-level').style.height = liquid + '%';
            if(liquid <= 0) { score += 50; updateHUD(); flashStage('#34d399'); liquid = 100; document.getElementById('liquid-level').style.height = liquid + '%'; lastTap = ''; }
        }
    };
}

// --- GIOCO: EQUILIBRIO ---
function initEquilibrio() {
    const stage = document.getElementById('game-stage');
    stage.innerHTML = `<div style="position:absolute; top:0; left:0; width:50%; height:100%; z-index:10;" onpointerdown="tilt(-1)" onpointerup="stopTilt()" onpointercancel="stopTilt()"></div>
        <div style="position:absolute; top:0; left:50%; width:50%; height:100%; z-index:10;" onpointerdown="tilt(1)" onpointerup="stopTilt()" onpointercancel="stopTilt()"></div>
        <div style="display:flex; justify-content:center; align-items:center; height:100%; width:100%; position:relative;"><div class="balance-bar"><div class="balance-safe"></div><div id="balance-cursor" class="balance-cursor" style="left:50%;"></div></div></div>
        <div style="position:absolute; bottom:20px; width:100%; text-align:center; color:var(--muted); font-size:12px;">Tieni premuto a Destra o Sinistra</div>`;
    let pos = 50, drift = 0, userTilt = 0, isStunned = false; 
    const cursor = document.getElementById('balance-cursor');
    window.tilt = function(dir) { if(!isStunned) userTilt = dir * 0.8; }; window.stopTilt = function() { userTilt = 0; };
    let drifter = setInterval(() => { if(gameActive && !isStunned) drift = (Math.random() - 0.5) * (0.8 + (score * 0.005)); }, 1000); gameIntervals.push(drifter);
    let tickCount = 0;
    function balanceLoop() {
        if(!gameActive) return;
        if(!isStunned) {
            pos += drift + userTilt;
            if(pos >= 35 && pos <= 65) { cursor.style.backgroundColor = 'var(--success)'; tickCount++; if(tickCount % 15 === 0) { score += 2; updateHUD(); } } 
            else { cursor.style.backgroundColor = 'var(--danger)'; }
            if(pos < 0 || pos > 100) { isStunned = true; cursor.innerText = "💥"; loseLife(); gameTimeouts.push(setTimeout(() => { if(!gameActive) return; pos = 50; drift = 0; userTilt = 0; cursor.innerText = ""; isStunned = false; }, 800)); }
            cursor.style.left = pos + '%';
        }
        gameAnimationFrames.push(requestAnimationFrame(balanceLoop));
    }
    gameAnimationFrames.push(requestAnimationFrame(balanceLoop));
}
